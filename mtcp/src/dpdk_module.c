...
/* packet waiting zone */
static struct rte_ring* tx_ring[MAX_CPUS] = {NULL};
...
/*----------------------------------------------------------------------------*/
void create_tx_ring(int cpu)
{
  char ring_name[NAME_SIZE];
  snprintf(ring_name, NAME_SIZE, "tx_ring_%d", cpu);
  tx_ring[cpu] = rte_ring_create(ring_name, TX_RING_SIZE, rte_socket_id(), 0);
  if (tx_ring[cpu] == NULL) {
    rte_exit(EXIT_FAILURE, "%s %s: %d\n",rte_strerror(rte_errno), __func__, 
      __LINE__);
  }
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
int dpdk_enqueue_pkts(struct mtcp_thread_context *ctx, int ifidx) {
  struct dpdk_private_context *dpc;
  int ret;
  int i;
  ...
  if (dpc->wmbufs[ifidx].len > 0) {
    struct rte_mbuf **pkts;
    int cnt = dpc->wmbufs[ifidx].len;
    pkts = dpc->wmbufs[ifidx].m_table;
    ret = rte_ring_enqueue_burst(tx_ring[ctx->cpu], (void *)pkts, cnt, NULL);
    /* time to allocate fresh mbufs for the queue */
    for (i = 0; i < dpc->wmbufs[ifidx].len; i++) {
      dpc->wmbufs[ifidx].m_table[i] = rte_pktmbuf_alloc(pktmbuf_pool[ctx->cpu]);
      /* error checking */
...
/*----------------------------------------------------------------------------*/
int
dpdk_pass_pkt(struct mtcp_thread_context *ctxt, int ifidx)
{
	struct dpdk_private_context *dpc;
#ifdef NETSTAT
	mtcp_manager_t mtcp;
	...
	ret = 0;
	void *pkt_ptr;
    ret = rte_ring_dequeue(tx_ring[ctxt->cpu], &pkt_ptr);
    if (ret == 0 ) {
      struct rte_mbuf* pkt_to_send;
      pkt_to_send = (struct rte_mbuf*)pkt_ptr;
...
#endif /* !ENABLE_STATS_IOCTL */
#endif
        ret = rte_eth_tx_burst(portid, ctxt->cpu, &pkt_to_send, cnt);
    }
...
/*----------------------------------------------------------------------------*/
int
dpdk_burst_pkts(struct mtcp_thread_context *ctxt, int ifidx)
{
	struct dpdk_private_context *dpc;
#ifdef NETSTAT
	mtcp_manager_t mtcp;
#endif
	int nb_sent, portid = CONFIG.eths[ifidx].ifindex;
    nb_sent = 0;
	dpc = (struct dpdk_private_context *)ctxt->io_private_context;
#ifdef NETSTAT
	mtcp = ctxt->mtcp_manager;
    ...
    int nb_dequeue = rte_ring_dequeue_burst(tx_ring[ctxt->cpu], (void*)pkts, MAX_PKT_BURST, NULL);
    if (nb_dequeue > 0 ) {
#ifdef ENABLE_STATS_IOCTL
#ifdef NETSTAT
		struct rte_eth_stats stats;
...
#endif
#endif /* !ENABLE_STATS_IOCTL */
        nb_sent = rte_eth_tx_burst(portid, ctxt->cpu, pkts, nb_dequeue);
#ifdef NETSTAT
		mtcp->nstat.tx_packets[ifidx] += nb_sent;
#ifdef ENABLE_STATS_IOCTL
		/* only pass stats after >= 1 sec interval */
		if (abs(mtcp->cur_ts - dpc->cur_ts) >= 1000 &&
		    likely(dpc->fd >= 0)) {
...
io_module_func dpdk_module_func = {
	...
	.burst_pkts		   = dpdk_burst_pkts,
	.enqueue_pkts		   = dpdk_enqueue_pkts,
  .pass_pkt          = dpdk_pass_pkt,
